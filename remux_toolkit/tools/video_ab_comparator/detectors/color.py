...
class ChromaShiftDetector(BaseDetector):
    ...
    def run(self, source: VideoSource) -> dict:
        ...
        if frame is None: return {'score': -1}
    ...
    def run(self, source: VideoSource) -> dict:
        ...
        with source as s:
            ...
            while (frame := s.read_frame()) is not None:
                ...
                # FORMULA RECALIBRATED: Less sensitive to rainbowing
...
class ColorCastDetector(BaseDetector):
    ...
    def run(self, source: VideoSource) -> dict:
        ...
        if not r_means: return {'score': 0, 'summary': 'Analysis failed'}
        color_dev = np.std([np.mean(b_means), np.mean(g_means), np.mean(r_means)])
        score = min(100, max(0, (color_dev - 2.0) * 10))
        return {'score': score, 'summary': f"Avg Deviation: {color_dev:.2f}", 'worst_frame_timestamp': source.info.duration / 2.0}
